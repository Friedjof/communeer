...
@router.post("/communities/{community_id}/renewals", response_model=RenewalCampaignSummaryOut)
...
def list_renewal_campaigns_route(
    ...
    community = get_community_or_404(db, community_id)
    return [_campaign_summary_out(db, c) for c in list_campaigns_for_community(db, community.id)]
